...
void UpdatePad(void)
{
	...
	for ( i=0 ; i<padCount ; i++ ) 
	{
		...
		//* Lスティック押し込み
		if (dijs.rgbButtons[10] & 0x80)	padState[i] |= BUTTON_LS;
		//* Rスティック押し込み
		if (dijs.rgbButtons[11] & 0x80)	padState[i] |= BUTTON_RS;

		//* 十字キー上
		if (dijs.rgdwPOV[0] == 0 ||
			dijs.rgdwPOV[0] == 4500 ||
			dijs.rgdwPOV[0] == 31500)	padState[i] |= BUTTON_UP;

		//* 十字キー下
		if (dijs.rgdwPOV[0] == 13500 ||
			dijs.rgdwPOV[0] == 18000 ||
			dijs.rgdwPOV[0] == 22500)	padState[i] |= BUTTON_DOWN;

		//* 十字キー左
		if (dijs.rgdwPOV[0] == 22500 ||
			dijs.rgdwPOV[0] == 27000 ||
			dijs.rgdwPOV[0] == 31500)	padState[i] |= BUTTON_LEFT;

		//* 十字キー右
		if (dijs.rgdwPOV[0] == 4500 ||
			dijs.rgdwPOV[0] == 9000 ||
			dijs.rgdwPOV[0] == 13500)	padState[i] |= BUTTON_RIGHT;
